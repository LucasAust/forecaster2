...
export function BudgetPieChart({ transactions: propTransactions }: { transactions?: any[] }) {
    ...
    // If we are using context transactions directly (no prop), we likely want "Current Month" view by default for this component
    const transactions = propTransactions ? propTransactions : rawTransactions.filter((tx: any) => {
    ...
    const categoryTotals = transactions.reduce((acc: any, tx: any) => {
        if (tx.amount > 0) { // Only expenses
            let category = tx.category?.[0];
            if (!category || category === "Uncategorized") {
                // Attempt fallback to name if generic
                // category = "Uncategorized"; 
                // Keep it simple for now, maybe the user really has uncategorized data.
                category = "Uncategorized";
            }
